...
const trustSignals = [
  'JWT-backed authentication and role controls',
  'IPFS-backed record storage for medical files',
  'Blockchain contract workflows for trust-critical data',
  'Frontend and backend tests already wired into the repo',
]
        ...
        <section className="grid gap-6 lg:grid-cols-[1.15fr,0.85fr]">
          <div className="brand-glass relative overflow-hidden rounded-[34px] px-7 py-8 sm:px-10 sm:py-10">
            ...
            <span className="brand-chip">
              ...
              Academic demo, production-style structure
            </span>
          ...
          <div className="flex flex-col gap-6">
            <div className="brand-glass rounded-[30px] p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-700">Operational Snapshot</p>
                  <h2 className="font-display mt-2 text-2xl font-semibold text-slate-900">Trust stack</h2>
                </div>
                <ChartBarSquareIcon className="h-8 w-8 text-brand-600" />
              </div>

              <div className="mt-6 space-y-4">
                {[
                  ['Frontend', 'React and Vite experience for role-based healthcare workflows'],
                  ['Backend', 'Express API with JWT auth, Mongoose models, and modular routes'],
                  ['Storage', 'MongoDB plus IPFS-backed record handling for file flows'],
                  ['Blockchain', 'Hardhat and Solidity contracts for integrity-sensitive operations'],
                ].map(([label, detail]) => (
                  <div key={label} className="rounded-2xl border border-slate-200 bg-slate-50/85 p-4">
            ...
            <div className="brand-glass rounded-[30px] p-6">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-700">Why it feels different</p>
              <div className="mt-4 space-y-3">
                {trustSignals.map((signal) => (
                  <div key={signal} className="flex items-start gap-3 rounded-2xl bg-white/90 px-4 py-3 shadow-sm shadow-slate-200/40">
                    <CheckBadgeIcon className="mt-0.5 h-5 w-5 shrink-0 text-success-600" />
